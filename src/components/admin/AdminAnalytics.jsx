...
import { BarChart3, Store, ClipboardCheck, CheckCircle2, AlertTriangle, Clock, TrendingUp } from 'lucide-react';
...
export default function AdminAnalytics() {
  ...
  const avgScore = shops.length ? Math.round(shops.reduce((sum, s) => sum + (s.compliance_score || 0), 0) / shops.length) : 0;
        ...
        <div className="bg-[#e8ecf1] rounded-3xl shadow-[8px_8px_16px_#c5c9ce,-8px_-8px_16px_#ffffff] p-6">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={municipalities} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
              ...
              <Tooltip contentStyle={{ background: '#e8ecf1', border: 'none', borderRadius: 12, boxShadow: '4px 4px 8px #c5c9ce' }} />
          ...
          <div className="space-y-2">
            {auditLogs.slice(0, 8).map(log => (
              <div key={log.id} className="flex items-center gap-3 px-3 py-2 rounded-xl bg-[#e8ecf1] shadow-[inset_2px_2px_4px_#c5c9ce,inset_-2px_-2px_4px_#ffffff]">
                <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: log.severity === 'critical' ? '#ef4444' : log.severity === 'warning' ? '#f59e0b' : '#0ea5e9' }} />
