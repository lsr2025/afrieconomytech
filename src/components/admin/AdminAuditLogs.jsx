...
import { FileText, Search, Filter, Download, AlertTriangle, Info, ShieldAlert } from 'lucide-react';
      ...
      <div className="bg-[#e8ecf1] rounded-3xl shadow-[8px_8px_16px_#c5c9ce,-8px_-8px_16px_#ffffff] p-6">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#e8ecf1] shadow-[4px_4px_8px_#c5c9ce,-4px_-4px_8px_#ffffff] text-sm text-slate-700 hover:shadow-[2px_2px_4px_#c5c9ce] transition-all"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="relative sm:col-span-1">
            ...
            <Input
              ...
              className="pl-9 bg-[#e8ecf1] border-0 shadow-[inset_4px_4px_8px_#c5c9ce,inset_-4px_-4px_8px_#ffffff] rounded-xl w-full"
            />
          </div>
          <select
            value={actionFilter}
            onChange={e => setActionFilter(e.target.value)}
            className="px-3 py-2 rounded-xl bg-[#e8ecf1] shadow-[inset_3px_3px_6px_#c5c9ce,inset_-3px_-3px_6px_#ffffff] text-slate-700 border-0 outline-none text-sm"
          >
          ...
          <div className="space-y-2 max-h-[600px] overflow-y-auto pr-1">
            {filtered.map(log => {
              ...
              return (
                <div
                  key={log.id}
                  className={`rounded-2xl border p-4 cursor-pointer transition-all ${SEVERITY_STYLES[log.severity || 'info']}`}
                ...
                >
                  ...
                  {isExpanded && (log.old_values || log.new_values) && (
                    <div className="mt-3 pt-3 border-t border-current/20 grid grid-cols-2 gap-3">
                      {log.old_values && (
                        <div>
                          <p className="text-xs font-semibold mb-1">Previous Values</p>
                          <pre className="text-xs bg-white/50 rounded-lg p-2 overflow-auto max-h-32">{JSON.stringify(log.old_values, null, 2)}</pre>
                        </div>
                      )}
                      {log.new_values && (
                        <div>
                          <p className="text-xs font-semibold mb-1">New Values</p>
                          <pre className="text-xs bg-white/50 rounded-lg p-2 overflow-auto max-h-32">{JSON.stringify(log.new_values, null, 2)}</pre>
                        </div>
                      )}
                    </div>
                  )}
